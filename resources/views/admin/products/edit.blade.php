<x-app-layout>
    ...
    <div class="py-12" x-data="{ 
                ...
                <div class="p-6 bg-gray-900 border-b border-gold/10">
                    <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" id="name" name="name" value="{{ old('name', $product->name) }}" class="mt-1 block w-full rounded-md border-gold/20 shadow-sm focus:border-gold focus:ring focus:ring-gold/20 focus:ring-opacity-50 @error('name') border-red-500 @enderror" required>
                            @error('name')
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="4" class="mt-1 block w-full rounded-md border-gold/20 shadow-sm focus:border-gold focus:ring focus:ring-gold/20 focus:ring-opacity-50 @error('description') border-red-500 @enderror" required>{{ old('description', $product->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" id="price" name="price" value="{{ old('price', $product->price) }}" step="0.01" min="0" class="mt-1 block w-full rounded-md border-gold/20 shadow-sm focus:border-gold focus:ring focus:ring-gold/20 focus:ring-opacity-50 @error('price') border-red-500 @enderror" required>
                            @error('price')
                                <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
        ...
        <div x-show="lightboxImage" 
            ...
            <div class="relative bg-gray-900 rounded-lg shadow-2xl p-4 max-w-2xl" @click.stop>
