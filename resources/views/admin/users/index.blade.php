<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gold leading-tight">Usuarios</h2>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-4">
                <form action="{{ route('admin.users.index') }}" method="GET" class="flex gap-2">
                    ...
                    @if(request('search'))
                        <a href="{{ route('admin.users.index') }}" class="px-4 py-2 bg-graphite text-silver rounded-lg hover:bg-graphite/80 transition">
                            Limpiar
                        </a>
                    @endif
                </form>
                ...
                <table class="min-w-full divide-y divide-gold/10">
                    <thead class="bg-gray-800">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gold uppercase tracking-wider">Email</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gold uppercase tracking-wider">Rol</th>
                    ...
                    <tbody class="bg-gray-900 divide-y divide-gold/10">
                        @forelse($users as $user)
                            <tr>
                                ...
                                <td class="px-6 py-4 text-silver">{{ $user->email }}</td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-right text-sm font-medium space-x-3">
                                    <a href="{{ route('admin.users.edit', $user) }}" class="px-2 py-1 inline-block bg-gold/10 text-gold rounded hover:bg-gold hover:text-ebony transition">Editar</a>
                                    @if(auth()->id() !== $user->id)
                                    ...
                                    @else
                                        <span class="text-silver text-sm">(tú)</span>
                                    @endif
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-6 text-center text-silver">No hay usuarios.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
